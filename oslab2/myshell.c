...
#define MAX_CMD_COUNT       64      //多条指令上限
...
*/
int getnum(char* string)    //输入字符串获取数值
{
    int ret = 0;
...
int exec_builtin(int argc, char**argv, int *fd) {
    ...
    if (strcmp(argv[0], "cd") == 0) {
        if (argc != 2) {
            ...
            if (ret) printf("cd指令路径错误!\n");
        }
        getcwd(current_path, MAX_BUF_SIZE);
        return 0;
    }
        ...
        int id = getnum(argv[argc-1]);
        if (id == getpid())     //不允许kill自己
            return 0;
        return -1;
    }
    else {
...
int main() {
    ...
    char *commands[128];
    char* multi_commands[MAX_CMDLINE_LENGTH];
    int multi_count = 0;
    int current;    //当前正在执行的指令
    int cmd_count;
    getcwd(current_path, MAX_BUF_SIZE);     //获取当前路径
    while (1) {
        /* TODO: 增加打印当前目录，格式类似"shell:/home/oslab ->"，你需要改下面的printf */
        if (multi_count == 0)        //当前命令已全部执行完毕
        {
            ...
            multi_count = split_string(cmdline, ";", multi_commands);
            current = 0;
        }
        strcpy(cmdline,multi_commands[current++]);        //取本次执行指令
        multi_count--;
